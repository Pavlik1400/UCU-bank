...
namespace auth {
    ...
    Service::Service(const nlohmann::json &cnf)
        ...
        , notification(cnf["notification"]["broker_address"].get<std::string>() + ":" + cnf["notification"]["broker_port"].get<std::string>(), cnf["notification"]["topic"].get<std::string>())
    {
        CUSTOM_LOG(lg, info) << '\n'
        << "auth::Service::Sercie(\n"
        << '\t' << "BasicMicroservice(\n"
    ...
    std::optional<std::string> Service::send_otp(const std::string & otp, const std::string & email)
    {
        ...
        try {
            notification.send(email+notification::email_sep+"SECRET-KEY: "+otp);
    ...
    Service::tfa_req_otp(const AuthDU & id_n_pwd)
    {
        ...
        case user::status::OK: break;
        case user::status::INVALID_PASSWORD: return {auth::status::INVALID_PASSWORD, {}};
        default: return {auth::status::GET_FAILED, {}};
        }
        auto [status, table]{user.get(phoneNo)};
