#include "user/UserMicroservice.h"
#include "user/utils.h"

void UserMicroservice::start() {
    db = client["bank"];
    users = db["users"];
    users.create_index(session, document{} << user::NAME << 1 << finalize);
...
user::status UserMicroservice::create(const user_t &user) {
    ...
    if (users.find_one(session, document{} << user::NAME << user.name << user::PHONE_NO << user.phoneNo << finalize)) {
        return user::USER_EXISTS;
    }

    auto doc = document{} << user::TYPE << user.type << user::NAME << user.name << user::PASSWORD << user.password
                          << user::DATE_OF_BIRTH << user.date_of_birth << user::PHONE_NO << user.phoneNo << user::EMAIL
                          << user.email << user::ADDRESS << user.address << user::GENDER << user.gender
                          << user::JOINING_DATE << generate_current_datetime() << finalize;
...
std::pair<user::status, user_t> UserMicroservice::get(const std::string &name, const std::string &phoneNo) {
    CUSTOM_LOG(lg, debug) << "Get call";
    auto result = users.find_one(session, document{} << user::NAME << name << user::PHONE_NO << phoneNo << finalize);
    user_t user;
...
user::status UserMicroservice::remove(const std::string &name, const std::string &phoneNo) {
    CUSTOM_LOG(lg, debug) << "Remove call";
    auto status = users.delete_one(session, document{} << user::NAME << name << user::PHONE_NO << phoneNo << finalize);
    return status ? user::status::OK : user::status::USER_DOESNT_EXIST;

}


user::status UserMicroservice::exists(const std::string &name, const std::string &phoneNo) {
    CUSTOM_LOG(lg, debug) << "Exists call";
    auto status = users.find_one(session, document{} << user::NAME << name << user::PHONE_NO << phoneNo << finalize);
    return status ? user::status::OK : user::status::USER_DOESNT_EXIST;
}

user::status UserMicroservice::valid_id(const std::string &id) {
    CUSTOM_LOG(lg, debug) << "Exists call";
    auto status = users.find_one(session,
                                 document{} << user::ID << bsoncxx::oid{bsoncxx::stdx::string_view{id}} << finalize);
    return status ? user::status::OK : user::status::USER_DOESNT_EXIST;
}

void UserMicroservice::register_methods() {
    ...
    rpc_server.bind("remove", [&](const std::string &name, const std::string &phoneNo) { return remove(name, phoneNo); });
    rpc_server.bind("exists", [&](const std::string &name, const std::string &phoneNo) { return exists(name, phoneNo); });
