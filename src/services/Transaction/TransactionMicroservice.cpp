...
TransactionMicroservice::TransactionMicroservice(const nlohmann::json &cnf) :
...
void TransactionMicroservice::start() {
    try {
...
void TransactionMicroservice::finish() {
    try {
...
transaction::status TransactionMicroservice::verify_transaction(const Transaction &tran) {
...
add_transaction_res TransactionMicroservice::add_transaction_to_db(
...
transaction::status TransactionMicroservice::update_transaction_status(
        unsigned long long entry_id,
        transaction::db_entry_status status
) {
    try {
...
transaction::status TransactionMicroservice::delete_transaction(unsigned long long int entry_id) {
    try {
        pq::work work(pq_connection.value());
...
tran_query_res TransactionMicroservice::get_transaction(const TransactionFilter &filter) {
    ...
    try {
        ...
        if (filter.description) sql += " and description LIKE '%" + non_tran_work.esc(filter.description.value) + "%'";
...
void TransactionMicroservice::register_methods() {
